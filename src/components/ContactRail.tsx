    ...
    >
      ...
      <div className="sm:hidden">
        ...
        <button
          ...
          className="inline-flex h-16 w-16 items-center justify-center rounded-l-card bg-brand-ink text-white shadow-lift"
        >
          <span className="sr-only">{open ? 'Hide contact options' : 'Show contact options'}</span>
          <span className={cn('scale-125 transition-transform duration-200', open && 'rotate-45')}>
...
function RailCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col rounded-l-card bg-brand-ink py-3 shadow-lift-hover">
      {children}
    </div>
  );
}
...
function RailLink({
  ...
  return (
    <a
      ...
      className="group/rail relative flex h-16 w-16 items-center justify-center text-white/85 transition-colors hover:text-accent-gold focus-visible:text-accent-gold"
    >
      <span className="sr-only">{label}</span>
      <span aria-hidden="true">{icon}</span>
      <span
        aria-hidden="true"
        className="pointer-events-none absolute right-full mr-1 origin-right scale-x-0 whitespace-nowrap rounded-l-pill bg-brand-ink py-2.5 pl-4 pr-2.5 text-sm font-semibold text-white opacity-0 transition-[transform,opacity] duration-200 ease-entrance group-hover/rail:scale-x-100 group-hover/rail:opacity-100 group-focus-visible/rail:scale-x-100 group-focus-visible/rail:opacity-100"
      >
        {short}
      </span>
    </a>
